...
for filename in glob.glob('D:/Imagenet/ILSVRC2012/ILSVRC/Data/CLS-LOC/val/*.JPEG'): #assuming jpg
  ...
  dfrows.append([filename[47:75], str_label_correct, str_label_orig, str_label_pert, torch.cuda.memory_stats('cpu')['active.all.current'], str(loop_i), str(execution_time), newf_k, average, fro])
  ...
  hybrows = []
  hybrows.append([filename[47:75], str_label_correct, str_label_orig, str_label_pert, torch.cuda.memory_stats('cpu')['active.all.current'], str(loop_i), str(execution_time), newf_k, average, fro])
  ...
  fgsmrows = []
  fgsmrows.append([filename[47:75], classes[int(correct)].split(',')[0], (classes[pred].split(',')[0]), (classes[pred_adv].split(',')[0]), torch.cuda.memory_stats('cpu')['active.all.current'], str(loop_i), str(execution_time), f_k, average, fro])
